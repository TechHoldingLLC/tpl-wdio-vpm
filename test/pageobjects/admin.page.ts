...
class AdminPage {
  ...
  public async searchedOrderDetails(): Promise<{
    ...
    const orderId = await $(
      "//table[contains(@class,'PatientList_table-data')]/tbody/tr/td[2]"
    ).getText();
    const orderPaymentStatus = await $(
      "//table[contains(@class,'PatientList_table-data')]/tbody/tr/td[8]"
    ).getText();
    await browser.pause(1000);
    const orderStatus = await $(
      "//table[contains(@class,'PatientList_table-data')]/tbody/tr/td[9]/span"
    ).getText();
