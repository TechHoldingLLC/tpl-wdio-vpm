...
class ProductDetail extends Page {
  ...
  public async productInfo(){
    const productDetails = await browser.$$("//div[contains(@class,'ProductDetail_accordion-wrapper')]")
    for await (const [index, collapseOption] of productDetails.entries()) {
      await collapseOption.click()
      const textArray = await collapseOption.$$("//div[contains(@class,'ProductDetail_accordion-content-contentfull')]")
  ...
  public get validationMobileOnSemaglutideForm(){
    return $("(//div[contains(@class,'postal-code-error TextError_errorText')])[3]")
  }
