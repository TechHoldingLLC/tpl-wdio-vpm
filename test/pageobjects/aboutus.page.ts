...
class AboutUsPage extends Page {
  ...
  public async validateAboutUsItemContent(expectedHeaders: string[]): Promise<boolean> {
    ...
    for (let i = 0; i < aboutUsContentHeaderList.length; i++) {
        ...
        const aboutUsContentHeaderText = await aboutUsContentHeaderList[i].getText()
        console.log(aboutUsContentHeaderText)
        actualAboutUsItemContentHeaders.push(aboutUsContentHeaderText)
    }

    console.log(`Actual About Us Content Header List "${actualAboutUsItemContentHeaders}"`)
    if (isEqual(actualAboutUsItemContentHeaders.sort(), expectedHeaders.sort())) {
        return true
    }
