...
// Helper function to get environment-specific login credentials
const getLoginData = (url: string, logindata: any): LoginData => {
  if (url.includes("qa")) return logindata.login_valid;
  if (url.includes("stage")) return logindata.stage_login_valid;
  return logindata.prod_login_valid;
};
...
describe("HomePage Features", () => {
  ...
  it("Website Main Pages: Verify Landing Page PE tile action - When iConsult user is logged-in", async () => {
    ...
    const language = await LoginPage.getLanguageFromUrl(url);
    const loginData = getLoginData(url, logindata);
    ...
    await expect(currentUrl).toContain("start-iconsult");
