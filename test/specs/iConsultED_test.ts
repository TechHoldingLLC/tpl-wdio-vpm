...
describe("iConsult Features", () => {
  ...
  it("C29653 Verify iConsult flow for Erectile dysfunction - Tadalafil Medicine", async () => {
    ...
    const iConsultEDData = JSON.parse(fs.readFileSync('./test/data/iConsultED.json', 'utf-8'))
    let loginData: any
    ...
    expect(Recommendation_medicine_title).toEqual(iConsultEDData.iConsultED_MedicineName)
    
    const language: string = await iConsult.getLanguageFromUrl(url)
    const expectedMedicineDescription = language === 'en' ? iConsultEDData.iConsultED_MedicineDescription : iConsultEDData.iConsultED_MedicineDescription_es
    ...
    const iConsult_SubscriptionPlan = await iConsultEDFlow.fetch_subscription_plan.getText()
    const iConsult_SubscriptionPlanAmount = await iConsultEDFlow.fetch_subscription_amount.getText()
    ...
    expect(await iConsult.iConsultPage).toHaveText(iConsultEDData.iConsultED_SummaryTitle)
    await browser.pause(5000)
    await expect(iConsultEDFlow.prescribed_medicine).toHaveText(iConsultEDData.iConsultED_MedicineName)
    await iConsult.addNewCard.scrollIntoView()
    await iConsult.cardSelection.click()
    await browser.pause(1000)
    await iConsult.submitOrder.click()

    const completionMsg = language === 'en' ? iConsultEDData.iConsultED_CompletionMsg : iConsultEDData.iConsultED_CompletionMsg_es
    await expect(iConsultEDFlow.iConsultCompletedMessage).toHaveText(completionMsg)
    await browser.pause(2000)
