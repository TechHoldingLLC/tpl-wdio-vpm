...
describe("Home Page Footer Links and Page Redirection", () => {
  ...
  it("C29963 Website Main Pages: Verify the Headers options available in the Home Page Footer.", async () => {
    ...
    const expectedFooterHeaders: string[] =
      language === "en" ? ["PRODUCTS", "LEGAL", "SUPPORT"] : ["PRODUCTOS", "LEGAL", "APOYO"];
  ...
  it("C29959 Website Main Pages: Verify the Terms of Use Page from the Home Page Footer.", async () => {
    ...
    const expectedTermsOfUsePageTitleText: string =
      language === "en"
    ...
    const expectedTermsOfUsePageHeaderText: string =
      language === "en"
        ? homePageData.terms_of_use_title
        : homePageData.terms_of_use_title_es;
    await expect(await homePage.title).toHaveText(
      expectedTermsOfUsePageHeaderText
    );
  ...
  it("C29958 Website Main Pages: Verify the Telehealth Consent Page from the Home Page Footer.", async () => {
    ...
    const expectedTeleHealthConsentPageTitle: string =
      language === "en"
        ? pagetitle.pg_title_Telehealth_Consent
        : pagetitle.pg_title_Telehealth_Consent_es;
    const expectedTeleHealthConsentPageHeaderText: string =
      language === "en"
        ? homePageData.telehealth_consent_title
        : homePageData.telehealth_consent_title_es;

    await expect(browser).toHaveUrl(
      expect.stringContaining("telehealth-consent")
    );
    await expect(browser).toHaveTitle(expectedTeleHealthConsentPageTitle);
    expect(await homePage.title).toHaveText(
      expectedTeleHealthConsentPageHeaderText
    );
