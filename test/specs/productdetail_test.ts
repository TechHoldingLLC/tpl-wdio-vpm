...
describe("Product Details Feature", () => {
  ...
  beforeEach(async () => {
    ...
    productDetaildata = JSON.parse(
      fs.readFileSync("./test/data/productDetail.json", "utf-8")
    );
  ...
  it("C29676 Website Main Pages: Verify Tadalafil Product page opens error-free", async () => {
    try {
      ...
      if (language === "en") {
        ...
        console.log(
          "Product Description: " +
            (await productdetailPage.productDescription.getText())
        );
        expect(await productdetailPage.productDescription.getText()).toEqual(
